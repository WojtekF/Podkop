...
///     a first-run postgres image pull.
/// </summary>
public sealed class OrchestrationSmokeFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        // Subscribed before StartAsync so even an instantly-exiting worker cannot be missed.
        var postgresHealthyTask = Model.Resources.Any(r => r.Name == "postgres")
            ? notifications.WaitForResourceHealthyAsync("postgres", window.Token)
            : null;
        var hasMigrations = Model.Resources.Any(r => r.Name == "migrations");

        await _app.StartAsync(window.Token);

        if (postgresHealthyTask is not null)
        {
            try
            {
                await postgresHealthyTask;
