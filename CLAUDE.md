...
4. **Guidance comments replace the content Claude would otherwise write.** The user is still learning Angular and SCSS, so each scaffolded file's comment block should teach, not just gesture: list the CSS hooks/classes and exact UI copy the colocated specs assert, and name the concrete constructs to reach for (`@if`/`@for`, signal reads like `finding().title`, which Angular Material module provides which element, `patchState`, `rxMethod` + `switchMap`, `takeUntilDestroyed`, SCSS nesting) — without writing the actual code.
