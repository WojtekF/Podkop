...
/// <summary>
///     The design-time half of the slice's persistence (issue #68): the EF command-line tooling
...
/// </summary>
public class FindingCommentsDesignTimeFactoryTests
{
    [Fact]
    public void The_tooling_can_build_the_context_with_nothing_orchestrated()
    {
        using var context = new FindingCommentsDbContextFactory().CreateDbContext([]);

        Assert.Equal("finding_comments", context.Model.GetDefaultSchema());
        Assert.NotNull(context.Model.FindEntityType(typeof(Comment)));
    }
    ...
    public void An_initial_migration_is_checked_in()
    {
        ...
        Assert.True(
            migrations.Length > 0,
            "The slice ships no migration, so a fresh database would come up without a "
            + "finding_comments schema: the migration worker applies migrations, it never creates "
            + "schemas on the fly. Generate the initial migration into this slice's own project "
            + "and check it in.");
    }
}
