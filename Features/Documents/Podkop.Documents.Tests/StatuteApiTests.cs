...
public class StatuteApiTests
{
    ...
    {
        // Version 3 is published (seeded) but its effective-from is still ahead of the pinned
        // clock: addressing it by number must 404 just like the current-document endpoint hides
        // it, so no route leaks an amendment before it takes force.
        using var factory = CreateFactory(At("2026-07-01T00:00:00Z"),
            Version(1, At("2025-01-01T00:00:00Z")),
            Version(2, At("2026-06-01T00:00:00Z")),
            Version(3, At("2099-01-01T00:00:00Z")));
        ...
        var response = await client.GetAsync("/api/statute/versions/3");

        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }

    [Fact]
    public async Task Version_becomes_readable_by_number_at_exactly_its_effective_instant()
    {
        // The by-number gate shares the current-document rule's inclusive boundary: at the
        // effective instant itself the amendment is already addressable. The far-future date
        // keeps the real clock on the wrong side, so only the injected clock can pass.
        using var factory = CreateFactory(At("2099-01-01T00:00:00Z"),
            Version(1, At("2025-01-01T00:00:00Z")),
            Version(2, At("2099-01-01T00:00:00Z")));
        using var client = factory.CreateClient();

        var statute = await client.GetFromJsonAsync<StatuteResponse>("/api/statute/versions/2");

        Assert.NotNull(statute);
        Assert.Equal(2, statute.Version);
        Assert.Equal(At("2099-01-01T00:00:00Z"), statute.EffectiveFrom);
    }
