...
///     #67): the real connection string only exists at run time inside Aspire, so design-time
...
public sealed class FindingsDbContextFactory : IDesignTimeDbContextFactory<FindingsDbContext>
{
    ...
    public FindingsDbContext CreateDbContext(string[] args)
    {
        ...
        options.UseFindingsPostgres(args.FirstOrDefault() ?? LocalDefaultConnectionString);
