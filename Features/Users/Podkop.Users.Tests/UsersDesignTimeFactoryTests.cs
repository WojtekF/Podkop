...
/// <summary>
///     The design-time half of the slice's persistence (issue #88): the EF command-line tooling
...
/// </summary>
public class UsersDesignTimeFactoryTests
{
    [Fact]
    public void The_tooling_can_build_the_context_with_nothing_orchestrated()
    {
        using var context = new UsersDbContextFactory().CreateDbContext([]);
    ...
    public void An_initial_migration_is_checked_in()
    {
        ...
        Assert.True(
            migrations.Length > 0,
            "The slice ships no migration, so a fresh database would come up without a users "
